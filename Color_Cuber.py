...
PATH_LOAD = "orig/hannah_and_me_square.jpg"
STARTITERATIONS = 100
ENDITERATIONS = 2000
STEPITERATIONS = 100
# ITERATIONS = 600
# PATH_SAVE = "mod/sunflower_Ellipse_" + str(ITERATIONS) + ".png"
#############################
...
class Section(object):
  def __init__(self, original, border):
    ...
    self.priority = (self.area ** 0.5) * (int(self.error))
...
class Original(object):
  ...
  def drawSections(self, path_to_save):
...
def main():
  ...
  for ITERATIONS in range(STARTITERATIONS, ENDITERATIONS, STEPITERATIONS):
    print(f"{ITERATIONS=}")
    ...
    PATH_SAVE = "mod/hannah_and_me_square_" + ("ELLIPSE_" if ELLIPSE else "") + str(ITERATIONS) + ".png"

    original.drawSections(PATH_SAVE)
